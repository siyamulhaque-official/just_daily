...
def extract_clean_content(entry, link):
    try:
        raw_html = ""
        # 1. Try grabbing full content directly from feed
        if 'content' in entry and len(entry.content) > 0:
            raw_html = entry.content[0].value
        elif 'description' in entry:
            raw_html = entry.description
            
        # 2. Fallback to web scraping if feed text is too short
        if len(raw_html) < 500:
            try:
                ...
                for entry in parsed_feed.entries:
                    try:
                        ...
                        if len(full_html) > 150:
                            ...
                            break # সফলভাবে নিউজ পেলে লুপ ব্রেক করবে
                    except Exception as entry_e:
                        print(f"Error processing entry: {entry_e}")
                        continue # এই নিউজে এরর হলে পরের নিউজে চলে যাবে
            except Exception as feed_e:
                print(f"Error processing feed: {feed_e}")
            break # একটি ফিড প্রসেস করেই থেমে যাবে
